...
export default function RetroactiveMilestoneInput() {
  ...
  const updateRetroactivePins = (value) => {
    ...
    setValue(
      ...
      (value || []).map((milestone) => {
        ...
        const previousCycle =
          parseInt(currentCycle) -
          (parseInt(currentMilestone) - parseInt(milestone));
  ...
  useEffect(() => {
    ...
    if ((retroactiveMilestones || []).length > 0)
      setCurrentRetroactive(
        (currentServices || [])
          .filter(
            (service) =>
              currentMilestone &&
              parseInt(service.milestone) !== parseInt(currentMilestone)
          )
  ...
  return (
    <Panel
      ...
      toggleable
      className={"mb-3"}
      headerTemplate={FieldsetHeader("Retroactive Service Pin Milestones")}
    >
      <div className="container">
        <div className="grid">
          <div className="col-12 form-field-container">
            <label htmlFor={`retroactive-milestones`}>
              Prior Unclaimed Service Pins
            </label>
            <MultiSelect
