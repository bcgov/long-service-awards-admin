...
export default function ReportList() {
    ...
    const reports = [
        {
            ...
            description: "All recipients registered for milestones at 25 years and up in current year.",
        ...
        },
        {
            ...
            description: "All recipients registered for service pins (including retroactive pins).",
            authorized: isAdmin
        }
    ];
    ...
    const onDownload = (data) => {
        ...
        const ts = Date.now();
        api.getReport(id, `${filename}-${ts}.${format}`)
            .catch(() => {
    ...
    return <>
        <DataTable
            value={reports}
            header={
            <Card className={'m-0 p-0'} title={'Reports'}></Card>}
            dataKey={'id'}
            stripedRows
            ...
            <Column
                ...
                body={(rowData) => {
                    return <Button
                        ...
                        icon={downloading[rowData.id] ? "pi pi-spin pi-spinner" : "pi pi-download"}
                        label={downloading[rowData.id] ? "Downloading..." : "Download CSV"}
