...
export default function AttendeesView({ data }) {
  console.log(data);
  const {
    type,
    label,
  ...
  return (
    <Panel className={"mb-2 mt-2"} header={"Attendees"} toggleable>
      <div className={"container"}>
        <div className={"grid"}>
          <div className={"col-6"}>Attendees type</div>
          ...
          <div className={"surface-hover col-12"}>
            {parse(description || "")}
          </div>
          ...
          <div className={"m-2 col-12"}>
            {(options || []).map((option) => {
              return (
                <div className={"grid border-1 m-1"} key={option.id}>
