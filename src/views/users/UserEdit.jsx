...
export default function UserEdit() {
  ...
  useEffect(() => {
    ...
    const filteredOrgs = (getValues("organizations") || []).map(
  ...
  useEffect(() => {
    ...
    setValue(
      "organizations",
      (currentOrganizations || []).map((orgName) => {
        return { user: userID, organization: orgName };
      })
    );
  ...
  return (
    <Panel className={"mb-3"} header={<>User Profile</>}>
